...
def main():
    ...
    # Campo de búsqueda
    search = st.text_input("Escribe el inicio de una película y presiona ENTER:", key="search_input")

    # Buscar películas al presionar ENTER
    if search:
        filtered_titles = filter_titles(search)
        if filtered_titles:
            st.session_state.auto_random_movies = data[data['title'].isin(filtered_titles)]
        else:
        ...
        if 'auto_random_movies' in st.session_state and not st.session_state.auto_random_movies.empty:
