...
class pln:
    ...
    def PROCESAR_SINOPSIS(texto):
        ...
        if isinstance(texto, str):
            ...
            for word in tokens:
                ...
                if word.isalnum() and word not in pln.stop_words:
                    filtered_tokens.append(word)
            
            # Paso 5: Aplicar stemming a las palabras filtradas
            stemmed_tokens = []
            for word in filtered_tokens:
                stemmed_word = pln.stemmer.stem(word)
    ...
    # Cargar datos 
    def CARGAR_DATOS(file_path, image_links_path):
        data_df = pd.read_csv(file_path)
        image_links = pd.read_csv(image_links_path)
        data_df = pd.merge(data_df, image_links, on="title", how="left")
        
        data_df['processed_synopsis'] = data_df['synopsis'].apply(pln.PROCESAR_SINOPSIS)
    ...
    def PELIS_SIMILARES(title, data_df, tfidf_matrix, top_n):
